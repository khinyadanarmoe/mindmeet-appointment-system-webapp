...
export default function SpecialityPage({ params }) {
  ...
  // Get speciality name from params
  useEffect(() => {
    const getParams = async () => {
      const resolvedParams = await params;
      setSpecialityName(resolvedParams.specialityName);
    };
    getParams();
  }, [params]);

  // Convert URL format back to display format
  const displaySpeciality = specialityName
    .replace(/-/g, " ")
      ...
      const filtered = therapists.filter(
        (therapist) =>
          therapist.speciality
            .toLowerCase()
            .includes(displaySpeciality.toLowerCase()) ||
          displaySpeciality
            .toLowerCase()
            .includes(therapist.speciality.toLowerCase())
      );
  ...
  return (
    <section className="max-w-6xl mx-auto px-4 py-10">
      <h2 className="text-xl font-bold text-purple-900 mb-8 text-center">
